...
async def callback_stats(callback: types.CallbackQuery):
    ...
    try:
        ...
        # Блок 1: Сегодня (решения = остановки)
        text_today = f"Сегодня: осознанных решений — {conscious}, попыток открыть TikTok — {attempts}."
        
        # Блок 2: За 7 дней
        # ИСПРАВЛЕНО: week_conscious считает события (остановки), а не дни.
        # Поэтому меняем "дней" на "остановок", чтобы не было противоречия с деревом.
        text_week = f"За 7 дней: осознанных остановок — {week_conscious}, серия — {tree.streak} дн."
        
        # Блок 3: Дерево (здесь хранятся именно уникальные дни)
